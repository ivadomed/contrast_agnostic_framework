...
contributes equally regardless of how many test cases it contributes. It is
a test \emph{across} tasks, not an average \emph{of} them: the main paper
deliberately prints no cross-task mean Dice, since averaging a whole-liver
Dice against a breast-lesion Dice yields a quantity with no clinical
referent, but a stratified test over the same tasks is still well defined
and is what the $p$ column reports. Per-setting comparisons (below) use the paired
