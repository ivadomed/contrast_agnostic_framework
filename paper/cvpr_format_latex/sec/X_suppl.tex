...
are not paired Wilcoxon tests, which is the more usual choice in this
literature, so we state the reason rather than leave it implicit.
...
statistic computed from those differences, $\text{macro}\Delta$ included,
which is what makes it applicable where the signed-rank test is not. We
