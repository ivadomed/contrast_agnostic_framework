...
eleven source-modality settings spanning six tasks --- abdominal, brain,
brain-tumour, dental and breast imaging --- together with four cross-dataset
external test cohorts, a single nnU-Net configuration trained with PALETTE
attains the best Dice on four of the six tasks and improves over all five
competing methods on a task-stratified test. It does not win everywhere, and
the two exceptions are informative rather than embarrassing: on mandible the
top three methods are statistically indistinguishable, and on breast the
Auglab pipeline we build on is genuinely ahead while the texture
intervention this paper argues for is, on that same task, among the
strongest effects we measure. A benchmark ranking and a causal account are
separate claims; we regard the
