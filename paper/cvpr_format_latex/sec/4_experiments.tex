...
\paragraph{Datasets.} We evaluate on four tasks drawn from six datasets
(\cref{tab:datasets}), spanning abdominal CT/MRI, brain MRI, and brain
tumor MRI. Every task (Brats-GLI, CHAOS, Open-MS, ON-Harmony) trains
on a single source modality and is tested on every other contrast of that
...
\caption{Per-task dataset roster. AMOS and SLIVER07 are cross-dataset
transfer targets only (``--'': never trained on). Subject counts,
acquisition protocols, and label provenance are in \cref{sec:suppl-data}.}
\label{tab:datasets}
\end{table}
...
SynthSeg-noEM   &  8.6 & 59.6 & 66.3 &  4.3 & 34.7 & 1.2e-48 \\
SynthSeg-EM     & 43.6 & 83.0 & 67.4 & 33.9 & 57.0 & 2.7e-13 \\
SRCSM           & 42.1 & 83.9 & 66.6 & 33.0 & 56.4 & 7.1e-12 \\
Auglab          & 46.7 & 83.3 & 67.4 & 34.3 & 57.9 & 0.0003 \\
\textbf{PALETTE (ours)} & \textbf{46.8} & \textbf{85.0} & \textbf{68.2} & \textbf{38.5} & \textbf{59.6} & --- \\
\bottomrule
\end{tabular}%
}
\caption{Task-level Dice (\%). Each column pools that task's held-out
contrasts and both source modalities, plus (CHAOS) an external
...
\cref{tab:meta-hd95}. $p$: Holm-corrected one-sided sign-flip test on the
macro-averaged per-case difference, stratified by task. PALETTE attains the
best Dice on every task and improves significantly over every competitor.}
\label{tab:meta}
\end{table}
...
(Holm-corrected across the four panels) -- a coarser, dataset-level
...
collapses. Three of four panels move significantly in the predicted
