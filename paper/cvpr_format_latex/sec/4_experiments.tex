...
surrounding Auglab configuration, whose two largest penalties anywhere in the
study fall on Mandible and Breast T2w --- the same two settings, as the
per-rung numbers in \cref{sec:suppl-persetting} show directly. A benchmark win and a causal
...
no-augmentation baseline is better still on Mandible. The cross-task test
does not show PALETTE ahead of Auglab on this metric ($p=1.00$). We give the
...
collapses. All three no-tissue-interface panels move significantly in the
predicted direction. Of the three tissue-interface panels, ToothFairy2's is
not significant either way --- the prediction for such a target coming true,
rather than a null result --- CHAOS's is a small but significant gain
($+0.72$), and ON-Harmony's is a significant \emph{worsening} (labelled accordingly, not a low $p$ defaulting to read
