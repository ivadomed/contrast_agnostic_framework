...
to the task-level comparison of \cref{tab:meta} and, via a cross-\emph{dataset}
rather than cross-\emph{contrast} OOD axis, to the causal-ablation ladder of
\cref{subsec:e-ablation} (\cref{tab:dissociation}) -- but not to the
per-setting breakdown of \cref{sec:suppl-persetting}, which requires the
cross-contrast axis the other four tasks have and this one does not.
...
\midrule
Baseline        & 21.7 & 29.3 & 31.1 & 21.0 & 33.3 & 27.3 & 1.0e-46 \\
SynthSeg-noEM   &  8.6 & 59.6 & 66.3 &  4.3 & 16.2 & 31.0 & 5.3e-74 \\
SynthSeg-EM     & 43.6 & 83.0 & 67.4 & 33.9 & 43.3 & 54.2 & 5.2e-12 \\
SRCSM           & 42.1 & 83.9 & 66.6 & 33.0 & 33.6 & 51.8 & 1.1e-32 \\
Auglab          & 46.7 & 83.3 & 67.4 & 34.3 & 40.9 & 54.5 & 3.4e-10 \\
\textbf{PALETTE (ours)} & \textbf{46.8} & \textbf{85.0} & \textbf{68.2} & \textbf{38.5} & \textbf{45.3} & \textbf{56.7} & --- \\
\bottomrule
\end{tabular}%
}
...
panel. No-tissue-interface tasks (BraTS-GLI, Open-MS, ATLAS-Liver-HCC): 8 of
16 curves improve significantly at the fill swap, 1 worsens significantly
(BraTS-GLI T2w$\to$T1n, discussed below), 7 are flat -- six of these are
ATLAS-Liver-HCC's individual cross-dataset evaluation streams, each too
small on its own to clear the corrected per-task threshold even though the
pooled effect across all six is significant (\cref{tab:dissociation}).
Tissue-interface tasks (CHAOS, ON-Harmony): only 1 of 16 improves and 1
worsens significantly -- the step is a non-event for 14 of 16 (88\%). Dice
numbers pooled per ladder in \cref{tab:dissociation}; HD95 in
...
Brats-GLI T1n   & no tissue interface & $\mathbf{+7.22}$ & 2.4e-25 \\
Brats-GLI T2w   & no tissue interface & $+0.60$ & 0.37 \\
ATLAS-Liver-HCC T1w & no tissue interface$^\ddagger$ & $+2.09$ & 3.5e-3 \\
\bottomrule
\end{tabular}
...
out-of-domain Dice, across all five headline tasks (both training
modalities each, except ATLAS-Liver-HCC's single modality). $p$ is a
Holm-corrected paired Wilcoxon test on this one step, pooled over each
task's OOD contrasts, across all nine rows. Two patterns emerge: large and
significant on the \emph{primary} modality of the no-tissue-interface
tasks, small on their secondary modality (still positive, but no longer
significant); and small-to-negative, rarely significant, on tissue-interface
tasks -- ON-Harmony's dense anatomical parcellation included
(\S\ref{subsec:e-ablation}). ATLAS-Liver-HCC, tested under a harder
cross-\emph{dataset} OOD axis rather than a held-out in-house contrast,
lands in between: positive and significant, but smaller than either
no-tissue-interface task's primary-modality effect -- consistent with the
same mechanism at a reduced, harder-generalization strength rather than a
counterexample to it. HD95 for this same step is reported in
...
$^\ddagger$Single training modality, so there is no second in-house
contrast to hold out; OOD instead pools case-level pairs across two
independent external cohorts (LiverHccSeg, LLD-MMRI-HCC) rather than a
held-out contrast within the training dataset -- see
\cref{subsec:e-ablation} and \cref{tab:datasets}.}
...
A fifth task lets us test the same dissociation on a third organ system.
ATLAS-Liver-HCC's target, hepatocellular carcinoma, is exactly the kind of
texture-only, no-interface label ON-Harmony's regions are not: a tumour
margin defined by enhancement pattern against surrounding liver
parenchyma, not a fixed anatomical boundary. It groups with BraTS-GLI and
Open-MS by the same criterion used above, and its fill-swap effect is
positive and significant, in the predicted direction
(\cref{tab:dissociation}). The magnitude is smaller than either
no-tissue-interface task's primary-modality effect, which we attribute to
the harder generalization axis rather than a weaker mechanism:
ATLAS-Liver-HCC's OOD is cross-\emph{dataset} (different institutions,
scanners and acquisition protocols entirely) rather than
cross-\emph{contrast} within one scanner protocol, so part of the effect
is plausibly absorbed by domain shift the other four tasks' ladders do not
have to cross. A third appearance-defined task, on a third organ system,
moving in the predicted direction strengthens the dissociation's
generality, even though its smaller size is a real, stated limitation
rather than a clean replication.
