...
$k$-means class count drawn per sample from $\{2,\dots,6\}$; Voronoi
sub-parcellation of each intensity class; signed affine coefficient
$\alpha \in \pm U(0.5, 2.0)$ applied per sub-region as in
\cref{eq:affine}; per-label affine decoupling applied to $50\%$ of labels.
The transform is applied on the GPU, on-the-fly, to a fixed $50\%$ of
training iterations (\emph{train050}); the remaining half of iterations use
the un-transformed real image.
