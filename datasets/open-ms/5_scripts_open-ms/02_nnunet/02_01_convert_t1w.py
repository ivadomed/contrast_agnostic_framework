#!/usr/bin/env python3
"""
Convert open_ms_data (1_BIDS) → nnU-Net raw dataset Dataset071_OpenMS_T1W.

T1w twin of 02_00_convert.py (which builds Dataset070_OpenMS_FLAIR). Training contrast =
T1w (single channel). The model is trained on the 22 train-pool patients' T1w and tested
CROSS-CONTRAST on the 8 held-out patients' FLAIR / T2W / T1W (all co-registered to FLAIR,
so one consensus mask serves every contrast). Mirrors the CHAOS second-contrast layout
(02_01_convert_t2spir.py). Reuses the SAME patient partition as FLAIR (partition.json).

Output under 2_nnUNet_open-ms/raw/Dataset071_OpenMS_T1W/:
  imagesTr/<patient>_0000.nii.gz        T1w, train pool
...
Run:  bash 02_nnunet/02_01_convert_t1w.sh   (or: python this file)
"""
...
OUT = DATASET_ROOT / "2_nnUNet_open-ms" / "raw" / "Dataset071_OpenMS_T1W"

# nnUNet test-input dir suffix -> BIDS suffix (per contrast). Training channel = T1w.
CONTRASTS = {"flair": "FLAIR", "t2w": "T2w", "t1w": "T1w"}
TRAIN_BIDS_SUFFIX = "T1w"
...
def main() -> None:
    ...
    # --- training set: T1w + mask (from BIDS) ---
    for pid in train_pool:
        _copy_image(_img(pid, TRAIN_BIDS_SUFFIX), OUT / "imagesTr" / f"{pid}_0000.nii.gz")
    ...
    dataset_json = {
        ...
        "description": "open_ms_data (Lesjak 2018, CC-BY) — brain MS lesion segmentation. "
                       "Train on T1w; cross-contrast test on FLAIR/T2W/T1W (co-registered).",
    ...
    print(f"Dataset071_OpenMS_T1W written → {OUT}")
