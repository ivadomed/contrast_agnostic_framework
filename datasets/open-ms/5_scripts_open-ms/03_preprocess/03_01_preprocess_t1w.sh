#!/usr/bin/env bash
# nnUNet plan & preprocess for Dataset071_OpenMS_T1W, then install our custom
# patient-level CV splits (same splits_final.json as FLAIR — same patient partition;
# nnUNet would otherwise generate its own at first training).
#
# Run this AFTER 02_nnunet/02_01_convert_t1w.sh has populated Dataset071_OpenMS_T1W.
#
# Usage: bash 03_01_preprocess_t1w.sh
set -euo pipefail
source "$(dirname "$0")/../00_utils/env_t1w.sh"
cd "${PROJECT_ROOT}"

DATASET_ID="${DATASET_ID:-71}"
...
run_job --name openms_preprocess_t1w --gpus 0 --cpus 8 --mem 32G --slot 0 --wait -- bash -c "
