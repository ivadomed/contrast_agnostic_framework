...
to open-ms (same mechanism, same validation_step/on_train_end structure in
...
nnUNetTrainerOpenMSAugLabDefault and nnUNetTrainerOpenMSAugLabValSynth are
untouched, and nothing changes for any wrapper script unless it explicitly
sets TRAINER=nnUNetTrainerOpenMSAugLabDualVal.

Implementation: reuses the two existing trainers' logic directly rather than
reimplementing it —
  - the primary/clean pass is nnUNetTrainerOpenMSAugLabDefault's OWN inherited
    ...
    nnUNetTrainerOpenMSAugLabDefault run would have produced;
  - the second, independent synth-only pass calls
    nnUNetTrainerOpenMSAugLabValSynth.validation_step UNBOUND on self (not
    ...
    nnUNetTrainerOpenMSAugLabValSynth run would have picked as ITS
...
open-ms_flair_baseline_kmeans_label_remap_affine_remap_train050_val000_<TS>)
...
nnUNetTrainerOpenMSAugLabValSynth requires) in addition to AUGLAB_PARAMS_GPU_JSON.
"""
...
class nnUNetTrainerOpenMSAugLabDualVal(nnUNetTrainerOpenMSAugLabDefault):
    ...
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
        ...
        if not val_json:
            raise RuntimeError(
                ...
                "nnUNetTrainerOpenMSAugLabDualVal."
            )
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]) -> None:
        ...
        # nnUNetTrainerOpenMSAugLabDefault run.
        ...
        # standalone nnUNetTrainerOpenMSAugLabValSynth run would compute, not a
        # reimplementation that could quietly drift from it.
        with torch.no_grad():
            synth_outputs = [
                nnUNetTrainerOpenMSAugLabValSynth.validation_step(self, next(self.dataloader_val))
