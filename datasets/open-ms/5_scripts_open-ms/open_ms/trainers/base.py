"""
nnUNetTrainerOpenMSBase — open-ms (brain MS lesion) dataset base.

Adds split validation + held-out-test contamination guard on top of
nnUNetTrainerFast. Place FIRST in every concrete open-ms trainer's MRO so its
do_split() wins (mirrors the CHAOS / ON-Harmony base trainers).

nnUNet case ids are the patient ids (`patientXX`). test_cases.json is
`{"test": ["patientYY", ...]}` (the held-out patients, never in train/val).

MRO example: nnUNetTrainerOpenMSV26_6_2
  → nnUNetTrainerOpenMSAugLabValSynth → ...AugLabDefault → nnUNetTrainerOpenMSBase
...
class nnUNetTrainerOpenMSBase(nnUNetTrainerFast):
    """open-ms split validation mixin (place first in the MRO)."""
    ...
    def do_split(self):
        ...
        splits_dir = Path(os.environ.get(
            "SPLITS_DIR",
            str(Path(__file__).resolve().parents[4] / "4_splits_open-ms"),
        ))
