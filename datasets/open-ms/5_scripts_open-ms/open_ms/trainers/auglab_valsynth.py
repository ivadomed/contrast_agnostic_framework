"""
nnUNetTrainerOpenMSAugLabValSynth — AugLab trainer with synthesis-only validation.
...
both set by the 04_train wrapper:
...
Used for auglabAug_v26_6_2_train025_val100 and similar. Identical structure to the
ON-Harmony / CHAOS AugLabValSynth trainers.
"""
...
from open_ms.trainers.auglab_default import nnUNetTrainerOpenMSAugLabDefault


class nnUNetTrainerOpenMSAugLabValSynth(nnUNetTrainerOpenMSAugLabDefault):
    """AugLab for open-ms with synth-only validation (val metrics on synthesised images)."""
    ...
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
        ...
        if not val_json:
            raise RuntimeError(
                ...
                "nnUNetTrainerOpenMSAugLabValSynth."
            )
