#!/usr/bin/env bash
# Aggregate a totalseg-pelvic results table from a YAML config (canonical shared driver —
# do NOT compute ad-hoc tables or p-values; the inline "sig. vs ref" column is auto-wired).
#   bash 06_02_aggregate_from_config.sh [configs/totalseg-pelvic_ct_01_results.yaml]
set -euo pipefail
