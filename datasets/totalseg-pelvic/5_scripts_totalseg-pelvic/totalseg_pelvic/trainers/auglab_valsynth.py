"""
nnUNetTrainerTotalsegPelvicAugLabValSynth — AugLab trainer with synthesis-only validation.
...
DualVal, see auglab_dualval.py) — used standalone by the ladder's rung 5 ("v26_6_2 alone",
real-fill, 04_XX_train_<contrast>_v26_6_2_train050_val100.sh). Direct port of
autopet.trainers.auglab_valsynth (itself ported from ambl / ON-Harmony / CHAOS / open-ms /
atlas-liver-hcc / toothfairy2), renamed.
"""
...
class nnUNetTrainerTotalsegPelvicAugLabValSynth(nnUNetTrainerTotalsegPelvicAugLabDefault):
    """AugLab for totalseg-pelvic with synth-only validation (val metrics on synthesised images)."""
    ...
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
        ...
        if not val_json:
            raise RuntimeError(
                ...
                "nnUNetTrainerTotalsegPelvicAugLabValSynth."
            )
