"""
nnUNetTrainerTotalsegPelvicAugLabDefault — AugLab GPU augmentation trainer for
totalseg-pelvic.

Also used for the SynthSeg variants (noEM, EM), srcsm, and every causal-ablation ladder
rung by pointing AUGLAB_PARAMS_GPU_JSON at the corresponding shared config — there is no
separate SynthSeg/srcsm/ladder trainer class, exactly as in every other dataset in this
project.

MRO: nnUNetTrainerTotalsegPelvicAugLabDefault
  -> nnUNetTrainerTotalsegPelvicBase   (do_split — anti-contamination guard)
...
class nnUNetTrainerTotalsegPelvicAugLabDefault(nnUNetTrainerTotalsegPelvicBase, nnUNetTrainerDAExtGPU):
    """AugLab GPU augmentation trainer for totalseg-pelvic (pelvic/hip musculoskeletal)
    segmentation.
