#!/usr/bin/env bash
# Predict with the MRI-trained ladder_v26_6_2_train050_val100 model, over ct/mri.
# Usage: bash 05_23_predict_mri_ladder_v26_6_2_train050_val100.sh <RUN_ID> [FOLD] [items...]
set -euo pipefail
source "$(dirname "$0")/../00_utils/env_mri.sh"
METHOD="v26_6_2_train050_val100"
TRAINER="nnUNetTrainerTotalsegPelvicAugLabValSynth"
CATEGORY="nnUNet"
DATASET_ID="130"   # fixed — imagesTs_* consolidated under Dataset130, see 05_01 header
source "$(dirname "$0")/05_01_predict_common.sh" "$@"
