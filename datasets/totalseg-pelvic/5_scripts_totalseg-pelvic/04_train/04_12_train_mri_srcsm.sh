#!/usr/bin/env bash
# Train srcsm (SRCSM SemRandConv-3D) on MRI — AugLab-category, reuses the auglab_default trainer. MEASURE per-epoch cost before packing folds on TamIA — do not assume any prior dataset's ratio transfers.
...
DATASET_ID="131"
DA_WORKERS=8
LOG_DIR="${RESULTS_DIR}/_logs/nnunet_totalseg_pelvic_mri_srcsm"
