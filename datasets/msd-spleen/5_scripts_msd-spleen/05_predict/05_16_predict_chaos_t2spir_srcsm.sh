#!/usr/bin/env bash
# Predict on MSD-SPLEEN CT with the CHAOS T2spir srcsm model (SRCSM SemRandConv-3D).
# Uses chaos Dataset061_CHAOS_MR_T2spir checkpoints (pre-exports override env.sh defaults).
# srcsm reuses the AugLab-default trainer (auglab category).
# Usage: bash 05_16_predict_chaos_t2spir_srcsm.sh [CHAOS_RUN_ID] [FOLD] [MODALITY ...]
set -euo pipefail
