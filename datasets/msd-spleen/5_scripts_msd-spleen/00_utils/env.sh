#!/usr/bin/env bash
# Source this file at the top of every msd-spleen pipeline script:
#   source "$(dirname "$0")/../00_utils/env.sh"   (from a step subdir)
#   source "$(dirname "$0")/00_utils/env.sh"       (from 5_scripts_msd-spleen root)
#
# MSD-SPLEEN is an EVALUATION-ONLY dataset (see datasets/msd-spleen/README.md). We
# never train here — we run models trained on `chaos` (MR T1-DUAL in-phase / T2-SPIR)
# over the Medical Segmentation Decathlon Task09_Spleen's 41 labeled CT volumes to
# measure MR->CT domain-randomization generalization on the spleen label. Hence there
# is NO 01_create_splits / 03_preprocess / 04_train stage (so no preprocessed/splits
# subdirs); the CHAOS_* vars point predict/evaluate at the chaos checkpoints.
#
# This is the msd-spleen "config": it sets the dataset-specific values, then sources
# datasets/00_commun_scripts/00_00_utils/common_env.sh for the standard paths.

DATASET_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"

export DATASET_NAME="msd-spleen"
...
# MSD-SPLEEN consumes chaos checkpoints. nnUNetv2_predict resolves the model from
...
# msd-spleen's env_t2spir.sh pre-export the T2spir chaos vars.
...
# spleen 4) — used by 06_evaluate to score the spleen label only (MSD-SPLEEN GT
# annotates the spleen alone).
...
BIDS_SUBDIR="msd-spleen"                  # → BIDS_ROOT under 1_BIDS_<name>/
