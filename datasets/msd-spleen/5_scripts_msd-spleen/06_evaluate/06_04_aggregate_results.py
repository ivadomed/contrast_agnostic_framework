#!/usr/bin/env python3
"""
Aggregate MSD-SPLEEN evaluation results across all chaos-trained methods.
...
(spleen label only), and writes a comparison Markdown table to the metrics root's
00_comparison.md.

This keeps its own spleen-only report layout, but the cross-fold statistic comes
from the shared commun aggregation core. The fold loader here is the spleen-only
variant (MSD-SPLEEN GT annotates the spleen alone), so it stays local.
...
DEFAULT_METRICS_ROOT = DATASET_ROOT / "8_results_msd-spleen" / "02_metrics"


def load_run(run_dir: Path) -> dict:
    """Return {metric: {fold: [values]}} for spleen label across all folds."""
...
def main() -> None:
    ...
    for run_dir in sorted(root.iterdir()):
        ...
        if data:
            ...
            print(f"  skip {run_dir.name}: no eval_all.csv with spleen rows found")
    ...
    lines = [
        "# MSD-SPLEEN — MR→CT Generalization Results",
        "",
        f"Generated: {now}  |  Experiments: {len(runs)}  |  Modality: CT  |  Label: spleen",
        "",
        "Values are **cross-fold mean ± std** (per-fold case means averaged across folds 0-2).",
        "All models are trained on **chaos MR** (cross-dataset inference).",
    ...
    for metric, title, fmt in (
        ...
        lines += [title, "", "| experiment | spleen | n_folds |", "|---|---|---|"]
