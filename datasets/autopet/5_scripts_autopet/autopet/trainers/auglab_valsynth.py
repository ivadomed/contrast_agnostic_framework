"""
nnUNetTrainerAutoPETAugLabValSynth — AugLab trainer with synthesis-only validation.
...
real-fill, 04_XX_train_<contrast>_v26_6_2_train050_val100.sh) — kept for parity with the
other datasets' trainer packages. Direct port of toothfairy2.trainers.auglab_valsynth
(itself ported from ambl / ON-Harmony / CHAOS / open-ms / atlas-liver-hcc), renamed.
"""
...
class nnUNetTrainerAutoPETAugLabValSynth(nnUNetTrainerAutoPETAugLabDefault):
    """AugLab for autopet with synth-only validation (val metrics on synthesised images)."""
