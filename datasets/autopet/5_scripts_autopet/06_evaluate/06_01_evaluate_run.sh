#!/usr/bin/env bash
# Evaluate one autopet OWN-model prediction run: every fold x FOUR items (ct, pet,
# psma_ct, psma_pet — own-contrast, cross-contrast, and cross-institution/cross-tracer,
# ALL in one pass since every item's ground truth is consolidated under Dataset120's raw
# tree, see 05_predict's build_test_inputs scripts), scoring the single "lesion" label
# against nnUNet_raw/Dataset120_.../labelsTs_<item>.
# Writes <item>_metrics.csv + eval_all.csv + eval_summary.md per fold under
#   8_results_autopet/02_metrics/autopet_model/<contrast>/<CATEGORY>_<RUN_ID>/fold{F}/
#
...
# CT-trained RUN_ID, env_pet.sh for a PET-trained one) — this determines where
# 8_results_autopet/02_metrics/autopet_model/<contrast>/... is written, NOT which items
# get scored (all 4 items are always scored, matching chaos's own multi-item pattern).
#
# Optional env:
...
DATASET_ID="120"   # fixed — imagesTs_*/labelsTs_* consolidated under Dataset120

ITEMS=(ct pet psma_ct psma_pet)
LABELS="tumor"   # matches the archive's own dataset.json label name — verified 2026-09-13, NOT "lesion"
...
run_job --name "autopet_eval_${CATEGORY}_${RUN_ID}${_OUT_SUFFIX}" --gpus 0 --cpus 8 \
...
for F in ${FOLDS}; do
    for item in ${ITEMS[*]}; do
        ...
        .venv/bin/python datasets/autopet/5_scripts_autopet/06_evaluate/06_00_evaluate.py \
