#!/usr/bin/env bash
# Predict on CIRRMRI-LIVER T1w+T2w with the CHAOS-TRAINED T2spir SynthSeg-noEM model model.
# CIRRMRI-LIVER has no model of its own — see 05_01_predict_common.sh.
# Usage: bash 05_15_predict_chaos_t2spir_synthseg_noEM.sh [CHAOS_RUN_ID] [FOLD] [MODALITY ...]
set -euo pipefail
