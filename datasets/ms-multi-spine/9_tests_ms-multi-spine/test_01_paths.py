"""Verify all paths for the ms-multi-spine dataset are correct after reorganisation."""
import subprocess
import sys
...
ENV_SH = DATASET_ROOT / "5_scripts_ms-multi-spine/00_utils/env.sh"
...
def test_env_sh_exports_required_vars():
    required = ["BIDS_ROOT", "nnUNet_raw", "nnUNet_preprocessed", "nnUNet_results",
...
def test_bids_symlink_exists():
    p = DATASET_ROOT / "1_BIDS_ms-multi-spine"
    assert p.exists(), f"BIDS dir/symlink not found: {p}"

def test_bids_symlink_resolves():
    p = DATASET_ROOT / "1_BIDS_ms-multi-spine"
    assert p.is_symlink(), f"Expected symlink at: {p}"
    assert p.resolve().exists(), f"Symlink target does not exist: {p.resolve()}"

def test_bids_root_has_subjects():
    p = Path(source_env(ENV_SH, "BIDS_ROOT"))
    subjects = list(p.glob("sub-*"))
    assert subjects, f"No sub-* dirs under BIDS_ROOT: {p}"
...
@pytest.mark.parametrize("dataset_id", ["Dataset104_MSMultiSpine"])
...
def test_conf_data_yaml_exists():
    p = DATASET_ROOT / "3_conf_ms-multi-spine" / "data.yaml"
    assert p.exists(), f"Missing: {p}"

def test_results_subdirs_exist():
    results = DATASET_ROOT / "8_results_ms-multi-spine"
