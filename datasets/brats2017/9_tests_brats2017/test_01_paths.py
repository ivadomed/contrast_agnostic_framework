"""Verify all paths for the brats dataset are correct after reorganisation."""
import json
import subprocess
import sys
...
ENV_SH = DATASET_ROOT / "5_scripts_brats/00_utils/env.sh"
...
def test_raw_root_has_brats_data():
    p = Path(source_env(ENV_SH, "RAW_ROOT"))
    contents = list(p.iterdir())
    assert contents, f"RAW_ROOT is empty: {p}"
...
@pytest.mark.parametrize("dataset_id", [
    "Dataset021_BraTSFlair_gen_raw",
    "Dataset022_BraTST1w_gen_raw",
    "Dataset023_BraTST2w_gen_raw",
    "Dataset024_BraTST1gd_gen_raw",
])
...
def test_conf_data_yaml_exists():
    p = DATASET_ROOT / "3_conf_brats" / "data.yaml"
    assert p.exists(), f"Missing: {p}"

def test_splits_file_exists():
    p = DATASET_ROOT / "4_splits_brats" / "brats_subject_split.json"
    assert p.exists(), f"Missing: {p}"
...
def test_results_subdirs_exist():
    results = DATASET_ROOT / "8_results_brats"
