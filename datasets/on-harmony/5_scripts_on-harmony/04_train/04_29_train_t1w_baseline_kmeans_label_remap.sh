#!/usr/bin/env bash
# BASELINE-ANCHORED CAUSAL LADDER (on-harmony T1w), rung 3 -- + label remap on top
# of K-means clustering. Shared, dataset-agnostic AugLab config already used by
# chaos's/brats2024-glioma's equivalent rung.
# Usage: bash 04_29_train_t1w_baseline_kmeans_label_remap.sh [RUN_ID]
source "$(dirname "$0")/../00_utils/env.sh"
METHOD="baseline_kmeans_label_remap"
TRAINER="nnUNetTrainerOnHarmonyAugLabDefault"
DA_WORKERS=0
LOG_DIR="/tmp/nnunet_on-harmony_t1w_baseline_kmeans_label_remap"
