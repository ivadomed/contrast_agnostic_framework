#!/usr/bin/env bash
# Train SynthSeg with EM intensity priors on ON-Harmony T2w, 2000 epochs.
# Uses nnUNetTrainerOnHarmonyAugLabDefault + Synthseg_EM AugLab config —
# same approach as brats2024-glioma / CHAOS (not BrainGenerator).
# Usage: bash 04_15_train_t2w_synthseg_EM.sh [RUN_ID]
source "$(dirname "$0")/../00_utils/env_t2w.sh"
METHOD="synthseg_EM"
TRAINER="nnUNetTrainerOnHarmonyAugLabDefault"
DA_WORKERS=0
LOG_DIR="/tmp/nnunet_on-harmony_t2w_synthseg_EM"
...
export AUGLAB_PARAMS_GPU_JSON="${_AUGLAB_CONFIGS}/transform_params_gpu_default01-23_Synthseg_EM.json"
