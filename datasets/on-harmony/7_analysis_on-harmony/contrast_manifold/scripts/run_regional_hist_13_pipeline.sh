...
SCRIPT="$PROJ/analysis/contrast_manifold/scripts/extract_features_regional_hist_13.py"
...
_normalize_and_analyze() {
    ...
    set_slot $SLOT "$PY" "$ANALYSIS" \
        --only "$RUN_NAME" \
        --mask-type "${MASK_TYPE}" \
        >> "/tmp/rh13_analysis_${VER}.log" 2>&1
