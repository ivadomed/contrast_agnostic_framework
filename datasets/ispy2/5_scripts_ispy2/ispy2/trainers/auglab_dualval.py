"""
nnUNetTrainerISPY2AugLabDualVal — ONE training run, TWO independent "best"
...
ispy2 (same mechanism — see that module's docstring, itself ported from
atlas_liver_hcc / open_ms, for the full rationale). OPT-IN ONLY:
nnUNetTrainerISPY2AugLabDefault and nnUNetTrainerISPY2AugLabValSynth are untouched, and
...
reimplementing it —
  - the primary/clean pass is nnUNetTrainerISPY2AugLabDefault's OWN inherited (stock
    nnUNetTrainer) validation_step and on_validation_epoch_end, completely unmodified,
    so checkpoint_best.pth is exactly what a standalone nnUNetTrainerISPY2AugLabDefault
    run would have produced;
  - the second, independent synth-only pass calls
    nnUNetTrainerISPY2AugLabValSynth.validation_step UNBOUND on self (not copy-pasted),
    and replicates nnU-Net's own EMA formula for its own best-so-far tracker, so it is
    selection-equivalent to what a standalone nnUNetTrainerISPY2AugLabValSynth run would
...
nnUNetTrainerISPY2AugLabValSynth requires) in addition to AUGLAB_PARAMS_GPU_JSON.
"""
...
from ispy2.trainers.auglab_valsynth import nnUNetTrainerISPY2AugLabValSynth
...
class nnUNetTrainerISPY2AugLabDualVal(nnUNetTrainerISPY2AugLabDefault):
    ...
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
        ...
        if not val_json:
            raise RuntimeError(
                ...
                "nnUNetTrainerISPY2AugLabDualVal."
            )
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]) -> None:
        ...
        # standalone nnUNetTrainerISPY2AugLabValSynth run would compute, not a
        # reimplementation that could quietly drift from it.
        with torch.no_grad():
            synth_outputs = [
                nnUNetTrainerISPY2AugLabValSynth.validation_step(self, next(self.dataloader_val))
