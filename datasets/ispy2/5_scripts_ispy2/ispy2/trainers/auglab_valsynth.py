"""
nnUNetTrainerISPY2AugLabValSynth — AugLab trainer with synthesis-only validation.
...
from ispy2.trainers.auglab_default import nnUNetTrainerISPY2AugLabDefault


class nnUNetTrainerISPY2AugLabValSynth(nnUNetTrainerISPY2AugLabDefault):
    """AugLab for ispy2 with synth-only validation (val metrics on synthesised images)."""
    ...
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
        ...
        if not val_json:
            raise RuntimeError(
                ...
                "nnUNetTrainerISPY2AugLabValSynth."
            )
