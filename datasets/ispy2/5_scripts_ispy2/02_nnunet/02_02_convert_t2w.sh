#!/bin/bash
# ispy2 -> nnU-Net raw, t2w modality (see 02_00_convert_lib.py for the design).
...
run_job --name ispy2_convert_t2w --gpus 0 --cpus 4 --mem 16G --time 04:00:00 \
    --log "${LOG_DIR}/convert_t2w_$(date +%Y%m%d_%H%M%S).log" --wait -- \
    "${REPO_ROOT}/.venv/bin/python" "${HERE}/02_02_convert_t2w.py"
