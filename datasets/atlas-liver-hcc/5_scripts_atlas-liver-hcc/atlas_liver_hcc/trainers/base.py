"""
nnUNetTrainerAtlasLiverHCCBase — atlas-liver-hcc (HCC liver tumour) dataset base.

Adds split validation + held-out-test contamination guard on top of
nnUNetTrainerFast. Place FIRST in every concrete atlas-liver-hcc trainer's MRO so its
do_split() wins (mirrors the CHAOS / ON-Harmony / open-ms base trainers).

nnUNet case ids are "atlas_XXX" (XXX = zero-padded original ATLAS patient index — see
01_create_splits/01_01_create_splits.py). test_cases.json is `{"test": ["atlas_YYY",
...]}` (the held-out patients, never in train/val).

MRO example: nnUNetTrainerAtlasLiverHCCAugLabDualVal
  -> ...AugLabValSynth -> ...AugLabDefault -> nnUNetTrainerAtlasLiverHCCBase
...
class nnUNetTrainerAtlasLiverHCCBase(nnUNetTrainerFast):
    """atlas-liver-hcc split validation mixin (place first in the MRO)."""
    ...
    def do_split(self):
        ...
        splits_dir = Path(os.environ.get(
            "SPLITS_DIR",
            str(Path(__file__).resolve().parents[4] / "4_splits_atlas-liver-hcc"),
        ))
