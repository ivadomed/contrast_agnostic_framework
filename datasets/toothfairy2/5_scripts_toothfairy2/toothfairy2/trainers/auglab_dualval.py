"""
nnUNetTrainerToothFairy2AugLabDualVal — ONE training run, TWO independent "best"
checkpoints (clean/val000 and synth-only/val100), with NO third/intermediate RUN_ID
ever appearing anywhere.

Direct port of ambl.trainers.auglab_dualval.nnUNetTrainerAMBLAugLabDualVal to
toothfairy2 (same mechanism — see that module's docstring, itself ported from
atlas_liver_hcc / open_ms, for the full rationale). OPT-IN ONLY:
nnUNetTrainerToothFairy2AugLabDefault and nnUNetTrainerToothFairy2AugLabValSynth are untouched, and
nothing changes for any wrapper script unless it explicitly sets
TRAINER=nnUNetTrainerToothFairy2AugLabDualVal.

Implementation: reuses the two existing trainers' logic directly rather than
reimplementing it —
  - the primary/clean pass is nnUNetTrainerToothFairy2AugLabDefault's OWN inherited (stock
    nnUNetTrainer) validation_step and on_validation_epoch_end, completely unmodified,
    so checkpoint_best.pth is exactly what a standalone nnUNetTrainerToothFairy2AugLabDefault
    run would have produced;
  - the second, independent synth-only pass calls
    nnUNetTrainerToothFairy2AugLabValSynth.validation_step UNBOUND on self (not copy-pasted),
    and replicates nnU-Net's own EMA formula for its own best-so-far tracker, so it is
    selection-equivalent to what a standalone nnUNetTrainerToothFairy2AugLabValSynth run would
    have picked as ITS checkpoint_best.pth — not an approximation of it.
...
ispy2_t1wce_auglabAug_v26_6_2_train050_val000_<TS>) — that IS the real training
...
nnUNetTrainerToothFairy2AugLabValSynth requires) in addition to AUGLAB_PARAMS_GPU_JSON.
"""
...
from toothfairy2.trainers.auglab_valsynth import nnUNetTrainerToothFairy2AugLabValSynth
...
def materialize_split_runs(output_folder: str, run_id: str, log=print) -> None:
    ...
    Identical logic to ambl.trainers.auglab_dualval.materialize_split_runs
    (itself ported from atlas_liver_hcc/open_ms) — kept as a separate copy here
    rather than a cross-dataset import, matching this project's convention of
    self-contained per-dataset trainer packages.
    """
...
class nnUNetTrainerToothFairy2AugLabDualVal(nnUNetTrainerToothFairy2AugLabDefault):
    ...
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
        ...
        if not val_json:
            raise RuntimeError(
                ...
                "nnUNetTrainerToothFairy2AugLabDualVal."
            )
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]) -> None:
        ...
        # standalone nnUNetTrainerToothFairy2AugLabValSynth run would compute, not a
        # reimplementation that could quietly drift from it.
        with torch.no_grad():
            synth_outputs = [
                nnUNetTrainerToothFairy2AugLabValSynth.validation_step(self, next(self.dataloader_val))
