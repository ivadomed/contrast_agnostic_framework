"""
nnUNetTrainerToothFairy2Base — toothfairy2 (I-SPY2, breast tumour) dataset base.

Adds split validation + held-out-test contamination guard on top of
nnUNetTrainerFast. Place FIRST in every concrete toothfairy2 trainer's MRO so its
...
MRO example: nnUNetTrainerToothFairy2AugLabDualVal
  -> ...AugLabValSynth -> ...AugLabDefault -> nnUNetTrainerToothFairy2Base
...
class nnUNetTrainerToothFairy2Base(nnUNetTrainerFast):
    """toothfairy2 split validation mixin (place first in the MRO)."""
    ...
    def do_split(self):
        ...
        splits_dir = Path(os.environ.get(
            "SPLITS_DIR",
            str(Path(__file__).resolve().parents[4] / "4_splits_toothfairy2"),
        ))
