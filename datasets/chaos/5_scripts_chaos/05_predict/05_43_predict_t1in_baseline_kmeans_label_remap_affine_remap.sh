...
# Voronoi), trained by 04_53 with nnUNetTrainerCHAOSAugLabDualVal — trains
...
#     bash 05_43_predict_t1in_baseline_kmeans_label_remap_affine_remap.sh <RUN_ID with _val000_>
#     bash 05_43_predict_t1in_baseline_kmeans_label_remap_affine_remap.sh <RUN_ID with _val100_>
# (no CHECKPOINT=/PREDICT_OUTPUT_SUBDIR= override needed — each mirror's own
# checkpoint_best.pth is already the right one.)
# Usage: bash 05_43_predict_t1in_baseline_kmeans_label_remap_affine_remap.sh <RUN_ID> [FOLD] [MODALITY ...]
set -euo pipefail
