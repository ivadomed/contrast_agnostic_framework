#!/usr/bin/env python3
"""
CHAOS T2spir causal-ablation ladder — baseline-anchored: each rung adds exactly one
...
(everything else held fixed). Mirrors open-ms's 06_12_ladder_summary.py pattern and
BraTS's 06_13_ladder_summary.py; non-headline run dirs live under ablations/ (see
CLAUDE.md's 02_metrics subdir convention) — reads them from there, writes its own
outputs there too.
...
(t1in, t1out, ct), excluding the training contrast (t2spir).
    ...
    for fold_dir in sorted(run_dir.glob("fold*")):
        ...
        with csv_path.open() as f:
            for row in csv.DictReader(f):
                c = row.get("modality") or row.get("group")
                try:
                    v = float(row[metric])
...
def main():
    series = {"dice": [], "hd95": []}
    lines = ["# CHAOS T2spir — causal ablation ladder", "",
    ...
    axes[0].plot(x, series["dice"], "o-", color="#eb6834")
    axes[0].set_xticks(x); axes[0].set_xticklabels(labels, fontsize=8, rotation=15, ha="right")
    axes[0].set_ylabel("OOD Dice (%)"); axes[0].set_title("CHAOS T2spir ladder — Dice")
    axes[0].grid(alpha=0.3)
    axes[1].plot(x, series["hd95"], "o-", color="#eb6834")
    axes[1].set_xticks(x); axes[1].set_xticklabels(labels, fontsize=8, rotation=15, ha="right")
    axes[1].set_ylabel("OOD HD95 mm (lower better)"); axes[1].set_title("CHAOS T2spir ladder — HD95")
    axes[1].grid(alpha=0.3)
    fig.tight_layout()
    png_path = ABLATIONS_ROOT / "ladder_t2spir.png"
