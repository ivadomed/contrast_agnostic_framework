#!/usr/bin/env python3
"""
Cross-experiment, cross-fold aggregation of CHAOS evaluation results.
...
Usage:
  ...
  python 06_aggregate_results.py --results_dir <nnUNet_results> --out_dir <03_aggregated> \
      --run_ids v26_6_2_20260608_003445 baseline_20260601_120000
"""
import argparse
import csv
import sys
...
def load_run(metrics_dir: Path) -> dict:
    ...
    02_metrics/nnUNet_v26_6_2_20260608_003445/.  It contains fold0/, fold1/, …
...
def build_fold0_heatmap(metrics_dir: Path) -> None:
    """Heatmap of fold-0 Dice per experiment × modality, saved alongside the
    ...
    try:
        ...
        print(f"  (matplotlib unavailable, skipping fold-0 heatmaps: {e})", file=sys.stderr)
        return
    ...
    for metric, title, prec, cmap in specs:
        ...
        ax.set_yticks(range(len(run_ids)), run_ids)
        ax.tick_params(axis='y', labelsize=7)
        ax.set_title(f"{title} — fold 0, cross-class mean")
        for i in range(mat.shape[0]):
            for j in range(mat.shape[1]):
                v = mat[i, j]
                if np.isfinite(v):
        ...
        ax.set_xlabel("modality"); ax.set_ylabel("experiment")
        fig.tight_layout()
        p = metrics_dir / f"02_02_heatmap_fold0_{metric}.png"
        fig.savefig(p, dpi=150, bbox_inches="tight")
        plt.close(fig)
        print(f"→ {p}")
...
def main():
    ...
    build_fold0_heatmap(metrics_dir)
