#!/usr/bin/env bash
# AUGLAB-ANCHORED CAUSAL LADDER, rung: K-means + per-anatomical-label remap +
# Voronoi spatial sub-parcellation (skip_sub_parc_prob=0.4, full AugLab
# default01-23 suite held fixed). Same shared AugLab config already used by
# open-ms's equivalent rung (dataset-agnostic).
#
...
#
# Usage:
#   bash 04_51_train_t1in_auglab_kmeans_label_remap_voronoi_dualval.sh
source "$(dirname "$0")/../00_utils/env.sh"

METHOD="auglab_kmeans_label_remap_voronoi_train025_dualval"
TRAINER="nnUNetTrainerCHAOSAugLabDualVal"
DATASET_ID="060"
DA_WORKERS="${DA_WORKERS:-0}"
LOG_DIR="${PREDICTIONS_ROOT}/${MODEL_TYPE}/${TRAINING_CONTRAST}/_logs/nnunet_chaos_t1in_auglab_kmeans_label_remap_voronoi_train025_dualval"
