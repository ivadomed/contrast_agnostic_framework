#!/usr/bin/env bash
# Train AugLab with the SynthSeg+EM GPU augmentation config on CHAOS MR T1-in.
# Same AugLab trainer as a future auglab_default, but AUGLAB_PARAMS_GPU_JSON points at
...
#
# Placement: 4 folds on ONLY slots 0 and 1 (GPUs 0,1), 2 folds packed per slot/GPU:
#   folds 0,1 → slot 0 / GPU 0 ; folds 2,3 → slot 1 / GPU 1. Slots 2,3 left free.
#
# Usage:
#   bash 04_03_train_synthseg_EM.sh                                       # auto: chaos_synthseg_EM_train100_val000_<TS>
#   bash 04_03_train_synthseg_EM.sh chaos_synthseg_EM_train100_val000_<TS>  # explicit RUN_ID to resume
...
DATASET_ID="060"
DA_WORKERS=8
LOG_DIR="/tmp/nnunet_chaos_synthseg_EM"
export nnUNet_compile=1
export NNUNET_NUM_EPOCHS="${NNUNET_NUM_EPOCHS:-500}"

# SynthSeg + EM augmentation config (shared AugLab config — not dataset-specific).
