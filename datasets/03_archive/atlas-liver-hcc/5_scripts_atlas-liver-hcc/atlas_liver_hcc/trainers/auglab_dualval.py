"""
nnUNetTrainerAtlasLiverHCCAugLabDualVal — ONE training run, TWO independent "best"
checkpoints (clean/val000 and synth-only/val100), with NO third/intermediate RUN_ID
ever appearing anywhere.

Direct port of open_ms.trainers.auglab_dualval.nnUNetTrainerOpenMSAugLabDualVal to
atlas-liver-hcc (same mechanism, same validation_step/on_train_end structure in both
datasets' AugLabDefault/AugLabValSynth trainers — see that module's docstring for the
full rationale). OPT-IN ONLY: nnUNetTrainerAtlasLiverHCCAugLabDefault and
nnUNetTrainerAtlasLiverHCCAugLabValSynth are untouched, and nothing changes for any
wrapper script unless it explicitly sets TRAINER=nnUNetTrainerAtlasLiverHCCAugLabDualVal.

Implementation: reuses the two existing trainers' logic directly rather than
reimplementing it —
  - the primary/clean pass is nnUNetTrainerAtlasLiverHCCAugLabDefault's OWN inherited
    (stock nnUNetTrainer) validation_step and on_validation_epoch_end, completely
    unmodified, so checkpoint_best.pth is exactly what a standalone
    nnUNetTrainerAtlasLiverHCCAugLabDefault run would have produced;
  - the second, independent synth-only pass calls
    nnUNetTrainerAtlasLiverHCCAugLabValSynth.validation_step UNBOUND on self (not
    copy-pasted), and replicates nnU-Net's own EMA formula (see
    nnunetv2/training/logging/nnunet_logger.py:92-96) for its own best-so-far tracker,
    so it is selection-equivalent to what a standalone
    nnUNetTrainerAtlasLiverHCCAugLabValSynth run would have picked as ITS
    checkpoint_best.pth — not an approximation of it.

Directory-level compatibility (on_train_end) — NO third-named directory ever exists:
the wrapper script's RUN_ID (passed in as $1 / auto-generated by train_common.sh) must
itself be named with "_val000_" already (e.g.
atlas-liver-hcc_t1w_auglabAug_v26_6_2_train050_val000_<TS>) — that IS the real training
directory, and its own checkpoint_best.pth is already exactly the clean/val000 result,
no extra step needed. At on_train_end, this trainer ADDITIONALLY materializes exactly
ONE sibling RUN_ID directory — same run_id with "_val000_" swapped for "_val100_" —
hard-linked (not copied: same content, zero extra disk) to a STANDARD
checkpoint_best.pth/checkpoint_final.pth under the trainer's normal fold_N/ layout,
sourced from THIS run's checkpoint_best_val100.pth. Predict/eval/resume scripts then
point at either RUN_ID exactly as if two separate trainings had happened, with no
CHECKPOINT=/PREDICT_OUTPUT_SUBDIR= special-casing anywhere else in the pipeline. See
materialize_split_runs() (module-level, reusable from a standalone script for runs
already in flight when this was added).

Requires AUGLAB_VAL_PARAMS_GPU_JSON (same synth-only VAL config
nnUNetTrainerAtlasLiverHCCAugLabValSynth requires) in addition to AUGLAB_PARAMS_GPU_JSON.
"""
...
def materialize_split_runs(output_folder: str, run_id: str, log=print) -> None:
    ...
    Identical logic to open_ms.trainers.auglab_dualval.materialize_split_runs —
...
class nnUNetTrainerAtlasLiverHCCAugLabDualVal(nnUNetTrainerAtlasLiverHCCAugLabDefault):
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]) -> None:
        ...
        # nnUNetTrainerAtlasLiverHCCAugLabDefault run.
        ...
        # standalone nnUNetTrainerAtlasLiverHCCAugLabValSynth run would compute, not a
        # reimplementation that could quietly drift from it.
        with torch.no_grad():
            synth_outputs = [
                nnUNetTrainerAtlasLiverHCCAugLabValSynth.validation_step(self, next(self.dataloader_val))
