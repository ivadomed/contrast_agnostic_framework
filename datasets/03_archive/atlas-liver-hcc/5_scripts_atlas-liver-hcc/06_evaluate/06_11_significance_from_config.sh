...
#   bash 06_11_significance_from_config.sh configs/atlas-liver-hcc_t1w_01_results.yaml \
#       --ref atlas-liver-hcc_t1w_auglabAug_v26_6_2_train050_val000_<TS>
#
# --ref: pass the EXACT reference run id explicitly whenever the run list also
# contains ablation variants that extend the same run id — see
# datasets/00_commun_scripts/00_03_evaluate/significance_from_config.py for the
# full statistical model (paired macro-permutation sign-flip test on per-case Dice,
# folds capped to 0-2 — see eval_folds.py).
set -euo pipefail
