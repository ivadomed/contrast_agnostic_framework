#!/usr/bin/env python3
"""
Aggregate KIDNEY-T2W evaluation results across all chaos-trained methods.
...
(kidney label only), and writes a comparison Markdown table to the metrics root's
00_comparison.md.

This keeps its own kidney-only report layout, but the cross-fold statistic comes
from the shared commun aggregation core. The fold loader here is the kidney-only
variant (this GT annotates the kidney alone, both sides merged into one label),
so it stays local.
...
DEFAULT_METRICS_ROOT = DATASET_ROOT / "8_results_kidney-t2w" / "02_metrics"


def load_run(run_dir: Path) -> dict:
    """Return {metric: {fold: [values]}} for kidney label across all folds."""
...
def main() -> None:
    ...
    for run_dir in sorted(root.iterdir()):
        ...
        if data:
            ...
            print(f"  skip {run_dir.name}: no eval_all.csv with kidney rows found")
    ...
    lines = [
        "# KIDNEY-T2W — MR->MR Generalization Results",
        "",
        f"Generated: {now}  |  Experiments: {len(runs)}  |  Modality: T2w MRI  |  Label: kidney",
        "",
        "Values are **cross-fold mean +/- std** (per-fold case means averaged across folds 0-2).",
        "All models are trained on **chaos MR** (cross-dataset inference, different scanners/cohort).",
        "",
    ]
    ...
    for metric, title, fmt in (
        ...
        lines += [title, "", "| experiment | kidney | n_folds |", "|---|---|---|"]
