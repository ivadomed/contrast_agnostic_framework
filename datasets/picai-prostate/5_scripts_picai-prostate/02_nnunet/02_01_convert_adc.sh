#!/usr/bin/env bash
# Build the ADC nnU-Net raw dataset (Dataset081_PICAI_ADC) from BIDS.
...
# Usage: bash 02_01_convert_adc.sh
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
source "${SCRIPT_DIR}/../00_utils/env_adc.sh"
...
run_job --name picai_convert_adc --gpus 0 --cpus 4 --mem 16G --time 00:30:00 \
    --log "${RESULTS_DIR}/_logs/picai_convert_adc.log" --wait -- bash -c "
    export BIDS_ROOT='${BIDS_ROOT}'
    export SPLITS_DIR='${SPLITS_DIR}'
    export nnUNet_raw='${nnUNet_raw}'
    cd '${PROJECT_ROOT}'
    .venv/bin/python '${SCRIPT_DIR}/02_00_convert.py' --contrast adc
"
