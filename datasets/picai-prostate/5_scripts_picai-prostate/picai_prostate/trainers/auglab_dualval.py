"""
nnUNetTrainerPICAIProstateAugLabDualVal — ONE training run, TWO independent "best"
...
to picai-prostate (same mechanism, same validation_step/on_train_end structure in
both datasets' AugLabDefault/AugLabValSynth trainers — see that module's
docstring for the full rationale). OPT-IN ONLY: a separate class —
nnUNetTrainerPICAIProstateAugLabDefault and nnUNetTrainerPICAIProstateAugLabValSynth are
untouched, and nothing changes for any wrapper script unless it explicitly
sets TRAINER=nnUNetTrainerPICAIProstateAugLabDualVal.

Implementation: reuses the two existing trainers' logic directly rather than
reimplementing it —
  - the primary/clean pass is nnUNetTrainerPICAIProstateAugLabDefault's OWN inherited
    ...
    nnUNetTrainerPICAIProstateAugLabDefault run would have produced;
  - the second, independent synth-only pass calls
    nnUNetTrainerPICAIProstateAugLabValSynth.validation_step UNBOUND on self (not
    ...
    nnUNetTrainerPICAIProstateAugLabValSynth run would have picked as ITS
...
picai-prostate_flair_baseline_kmeans_label_remap_affine_remap_train050_val000_<TS>)
— that IS the real training directory, and its own checkpoint_best.pth is
already exactly the clean/val000 result, no extra step needed. At
on_train_end, this trainer ADDITIONALLY materializes exactly ONE sibling
RUN_ID directory — same run_id with "_val000_" swapped for "_val100_" —
hard-linked (not copied: same content, zero extra disk) to a STANDARD
checkpoint_best.pth/checkpoint_final.pth under the trainer's normal fold_N/
layout, sourced from THIS run's checkpoint_best_val100.pth. Predict/eval/
resume scripts then point at either RUN_ID exactly as if two separate
trainings had happened, with no CHECKPOINT=/PREDICT_OUTPUT_SUBDIR=
special-casing anywhere else in the pipeline. See materialize_split_runs()
(module-level, reusable from a standalone script for runs already in flight
when this was added).

Requires AUGLAB_VAL_PARAMS_GPU_JSON (same synth-only VAL config
nnUNetTrainerPICAIProstateAugLabValSynth requires) in addition to AUGLAB_PARAMS_GPU_JSON.
"""
...
class nnUNetTrainerPICAIProstateAugLabDualVal(nnUNetTrainerPICAIProstateAugLabDefault):
    ...
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
        ...
        if not val_json:
            raise RuntimeError(
                ...
                "nnUNetTrainerPICAIProstateAugLabDualVal."
            )
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]) -> None:
        ...
        # nnUNetTrainerPICAIProstateAugLabDefault run.
        ...
        # standalone nnUNetTrainerPICAIProstateAugLabValSynth run would compute, not a
        # reimplementation that could quietly drift from it.
        with torch.no_grad():
            synth_outputs = [
                nnUNetTrainerPICAIProstateAugLabValSynth.validation_step(self, next(self.dataloader_val))
