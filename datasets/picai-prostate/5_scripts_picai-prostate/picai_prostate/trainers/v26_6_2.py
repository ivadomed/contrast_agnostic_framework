"""
nnUNetTrainerPICAIProstateV26_6_2 — V26_6_2 contrast synthesis for picai-prostate (csPCa lesion).

Runs the AugLab GPU ImageContrastV26_6_2GPUTransform (K-means/Voronoi contrast
synthesis, per-patch) — the same backend as CHAOS / BraTS / ON-Harmony. Synthesis
probabilities come from the config JSONs the 04_train wrapper exports (NOT class
attributes):
  AUGLAB_PARAMS_GPU_JSON      → train config (v26-only spatialDA, or full auglabAug+v26)
  AUGLAB_VAL_PARAMS_GPU_JSON  → val: transform_params_gpu_VALsynthonly_ImageContrastV26_6_2GPUTransform.json

MRO: nnUNetTrainerPICAIProstateV26_6_2
  → nnUNetTrainerPICAIProstateAugLabValSynth (synth-only validation)
  → nnUNetTrainerPICAIProstateAugLabDefault  (AugLab GPU train transforms)
  → nnUNetTrainerPICAIProstateBase           (do_split guard)
  → nnUNetTrainerDAExtGPU
"""
from __future__ import annotations

from picai_prostate.trainers.auglab_valsynth import nnUNetTrainerPICAIProstateAugLabValSynth


class nnUNetTrainerPICAIProstateV26_6_2(nnUNetTrainerPICAIProstateAugLabValSynth):
    """V26_6_2 for picai-prostate via the AugLab contrast transform (see module docstring)."""
