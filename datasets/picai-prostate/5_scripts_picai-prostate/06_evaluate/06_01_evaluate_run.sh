#!/usr/bin/env bash
# Evaluate one picai-prostate prediction run: every fold × every contrast (t2w/adc/hbv),
...
#   8_results_picai-prostate/02_metrics/<MODEL_TYPE>/<TRAINING_CONTRAST>/<CATEGORY>_<RUN_ID>/fold{F}/
...
#   (ADC-trained runs: use 06_02_evaluate_adc.sh, which sets DATASET_ID=81 + the ADC env)
...
# tamia keeps predictions/metrics on $SCRATCH — must land AFTER env.sh, which
# exports PREDICTIONS_ROOT/METRICS_ROOT unconditionally.
...
# + dataset.json. Defaults to 80 (T2W-trained); the ADC wrapper (06_02) pre-exports 81.
# GT masks are byte-identical across the T2W/ADC datasets (all three contrasts share one
# resampling grid, so one mask serves them all), so this only affects provenance/paths,
# not the scores. Still pass it explicitly for ADC — the silent-wrong-GT trap that bit
# chaos/brats (CLAUDE.md 'Common gotchas') applies to the paths here too.
...
run_job --name "picai_eval_${CATEGORY}_${RUN_ID}${_OUT_SUFFIX}" --gpus 0 --cpus 8 --mem 16G --time "${EVAL_TIME:-1:00:00}" \
...
for F in ${FOLDS}; do
    for item in ${ITEMS[*]}; do
        ...
        .venv/bin/python datasets/picai-prostate/5_scripts_picai-prostate/06_evaluate/06_00_evaluate.py \
