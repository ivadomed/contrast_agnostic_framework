#!/usr/bin/env bash
# nnUNet plan & preprocess for Dataset081_PICAI_ADC, then install our custom
# patient-level CV splits (nnUNet would otherwise generate its own at first training).
# Usage: bash 03_01_preprocess_adc.sh
set -euo pipefail
...
run_job --name picai_preprocess_adc --gpus 0 --cpus 32 --mem 128G --time 04:00:00 --slot 0 --wait -- bash -c "
