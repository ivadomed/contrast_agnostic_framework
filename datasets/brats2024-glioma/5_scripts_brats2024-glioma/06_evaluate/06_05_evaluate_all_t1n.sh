#!/usr/bin/env bash
# Evaluate all T1n experiments listed in a config file, then aggregate.
# Run IDs are read from the config — no separate list to maintain.
#
# Usage:
#   bash 06_05_evaluate_all_t1n.sh [config.yaml]
#
# Defaults to configs/brats_t1n_01_results.yaml.
# Run this once T1n inference (05_predict scripts) has completed for all experiments.

set -euo pipefail
source "$(dirname "$0")/../00_utils/env.sh"
HERE="$(cd "$(dirname "$0")" && pwd)"

CONFIG="${1:-${HERE}/configs/brats_t1n_01_results.yaml}"
...
echo "[$(date '+%H:%M:%S')] Evaluating ${#RUNS[@]} T1n experiments from $(basename "$CONFIG")"
