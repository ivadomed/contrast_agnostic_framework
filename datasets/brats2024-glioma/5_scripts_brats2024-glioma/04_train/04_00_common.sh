...
#   DA_WORKERS   number of batchgenerators DA workers (64 for baseline, 0 for synth — slot provides 64 CPUs each)
...
#     `timeout`, and do NOT `nohup … &` it. Each fold is an independent set_slot job
#     (own systemd slice) that survives after this script exits; the script only has
#     to stay alive through the ~45s launch stagger. By default it does NOT `wait`
#     (fire-and-exit), so just run it foreground from the Bash tool:
#       bash 04_0X_train_<method>.sh <RUN_ID>      # returns after the stagger; folds keep running
#     PITFALLS that cost hours here:
#       - `timeout N bash …` KILLS the folds: timeout SIGTERMs the whole process
#         group at N seconds and that reaches the set_slot jobs → silent death ~N s in.
#       - `nohup bash … &` is reaped by the Bash-tool slot wrapper before it fires
#         anything (empty log, no error).
#     (Set LAUNCH_WAIT=1 to block until all folds finish — only for non-Bash-tool use.)
#  2. GPU PINNING is automatic here (CUDA_VISIBLE_DEVICES=<fold> below). Do NOT
#     hand-roll set_slot launches without it — all folds pile onto GPU 0 and run
#     ~4× slower (serialized). Always launch through this script.
#  3. EXTEND past the original length by raising NNUNET_NUM_EPOCHS. It is the PolyLR
#     horizon: resuming a finished run with the same value trains 0 epochs, and a
#     stale horizon makes the LR go negative→complex and crash. (fast.py now sets
#     num_epochs BEFORE configure_optimizers so the scheduler gets the real value.)
#  4. WANDB resumes the same run automatically via each fold's wandb/ dir. If that
#     dir was deleted, pass RESUME_WANDB_IDS=<id per fold>; the patched logger
#     (nnunet_logger.py: nnUNet_wandb_run_id + allow_val_change) resumes by id from
#     the cloud. Don't delete the wandb/ dirs in the first place.
#  5. After many launch/kill cycles, set_slot may briefly return exit 144 (slices not
#     released). Wait ~10s after pkill before relaunching.
...
# Stagger between fold launches: simultaneous set_slot→sudo ml_job→systemd-run
# calls race during slice setup and die silently right after the nnU-Net banner.
# A few seconds between launches lets each slice come up cleanly.
...
if [ -n "${FOLD_SLOT_GPU:-}" ]; then
    ...
    # DDP: only 2 folds fit at a time, so rounds are inherently sequential → must wait.
    for ROUND in 0 1; do
