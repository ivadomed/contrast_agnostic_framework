"""
nnUNetTrainerBraTS2024GliomaAugLabValSynth — AugLab trainer that applies SYNTHESIS in validation.
...
Used by any wrapper that points AUGLAB_VAL_PARAMS_GPU_JSON at a synth-only config.
"""
...
class nnUNetTrainerBraTS2024GliomaAugLabValSynth(nnUNetTrainerBraTS2024GliomaAugLabDefault):
    """AugLab for BraTS with real synth-only validation (val metrics on synth images)."""
    ...
    def _log_wandb_images(self, epoch: int = 0) -> None:
        """4-panel: T1w | train(full aug) or val(synth-only) | GT | Prediction.
        ...
        try:
            ...
            for tag, loader, tfm, aug_title in [
                ...
                ax[0].imshow(raw, cmap="gray");                   ax[0].set_title("T1w");        ax[0].axis("off")
            ...
            import traceback
            print(f"[WandB] BraTS auglab valsynth image log failed: {e}\n{traceback.format_exc()}")
