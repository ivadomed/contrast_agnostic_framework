#!/usr/bin/env python3
"""
Convert BraTS 2024 glioma BIDS data to nnUNet raw format — T2f (FLAIR) only.

Produces Dataset053_BraTS2024GliomaT2f with a single channel (FLAIR / T2f).
Parallel to 02_01_convert_t1n.py (Dataset051, T1n) / 02_02_convert_t2w.py
(Dataset052, T2w) — same case set, same label source, single channel replaced.

Usage:
  python 02_03_convert_t2f.py [--dataset-id 053] [--jobs N]

Reads from: ../../1_BIDS_brats2024-glioma/glioma-brain-brats2024/
Writes to:  ../../2_nnUNet_brats2024-glioma/raw/Dataset053_BraTS2024GliomaT2f/
"""
...
def convert_subject(sub_name: str, images_tr: Path, labels_tr: Path) -> str:
    ...
    src = anat_dir / f"{sub_name}_FLAIR.nii"
    if not src.exists():
        raise FileNotFoundError(f"Missing FLAIR: {src}")
...
def main():
    ...
    parser.add_argument("--dataset-id", type=int, default=53)
    parser.add_argument("--jobs", type=int, default=16)
    args = parser.parse_args()

    ds_name   = f"Dataset{args.dataset_id:03d}_BraTS2024GliomaT2f"
    ...
    print(f"Converting {len(subjects)} subjects (T2f/FLAIR only) with {args.jobs} workers …")
    ...
    dataset_json = {
        "name": "BraTS2024GliomaT2f",
        "description": "BraTS 2024 Glioma — T2f/FLAIR only (synthesis comparison from FLAIR input)",
