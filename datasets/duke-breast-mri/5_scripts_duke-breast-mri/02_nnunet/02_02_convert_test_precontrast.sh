...
# nnU-Net set (291 patients, precontrast/DCE-native only). Submitted through run_job:
# ~582 volumes (pre + post-contrast) are copied/reoriented -- well past the
# login-node exception in CLAUDE.md.
#
#   bash 02_02_convert_test_precontrast.sh
#
...
run_job --name duke_test_precontrast --gpus 0 --cpus 4 --mem 16G --time 02:00:00 \
    --log "${LOG_DIR}/convert_test_precontrast_$(date +%Y%m%d_%H%M%S).log" --wait -- \
    "${REPO_ROOT}/.venv/bin/python" "${HERE}/02_02_convert_test_precontrast.py"
