#!/usr/bin/env bash
# Evaluate one atlas-liver-hcc-model run on LLD-MMRI-HCC (T2w + DWI): Dice + HD95 for
# the tumour/lesion label only, per item present in the prediction dir.
#
# LLD-MMRI-HCC GT annotates the lesion alone as GT label 1. atlas-liver-hcc models emit
# tumour as label 2 (their own 3-class label space: background 0, liver 1, tumour 2), so
# pred and GT do NOT share a numeric id here — this uses --label_map (cross-label-space,
# same mechanism msd-spleen uses for chaos's spleen id 4 -> its own GT id 1). This
# dataset has no liver-organ mask (lesion-only annotation), so only "tumour" is scored,
# never "liver".
#
# Predictions are expected under:
#   PREDICTIONS_ROOT/{ATLAS_MODEL_TYPE}/{ATLAS_TRAINING_CONTRAST}/{CATEGORY}/{RUN_ID}/fold{k}/{t2wi,dwi}/
# GT is under:
#   2_nnUNet_lld-mmri-hcc/raw/labelsTs_{t2wi,dwi}/
# Metrics written to:
#   METRICS_ROOT/{ATLAS_MODEL_TYPE}/{ATLAS_TRAINING_CONTRAST}/{CATEGORY}_{RUN_ID}/fold{k}/{t2wi,dwi}_metrics.csv
#                                                                                             eval_all.csv
#                                                                                             eval_summary.md
#
# Usage:
...
eval_fold() {
    ...
    for d in "$PRED_ROOT"/*/; do
        ...
        items+=("$it")
        run_job --name "lldmmrihcc_eval_${RUN_ID}_fold${F}_${it}" --gpus 0 --mem 32G --time 00:45:00 --slot "${SLOT}" --wait -- \
    ...
    .venv/bin/python "${PROJECT_ROOT}/datasets/00_commun_scripts/00_03_evaluate/summarize_fold.py" \
        ...
        --title-suffix " | LLD-MMRI-HCC MRI | tumour only" \
        --groups "${items[@]}"
