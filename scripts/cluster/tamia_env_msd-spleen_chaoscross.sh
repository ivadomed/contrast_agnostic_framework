...
# MSD-SPLEEN's own paths (inputs for cross-mode predict + this dataset's own output tree).
# METRICS_ROOT is intentionally left at common_env.sh's default (under $PROJECT, NOT
# scratch) — same reasoning as sliver07: prediction NIfTI volumes are large/regenerable
# (scratch, purge-able), but the small per-fold metrics CSVs are precious and land in
# the backed-up $PROJECT repo checkout so they can be copied back to Vulcan afterward.
export nnUNet_raw="$SCRATCH/msd-spleen/2_nnUNet_msd-spleen/raw"
export PREDICTIONS_ROOT="$SCRATCH/msd-spleen/8_results_msd-spleen/01_predictions"
