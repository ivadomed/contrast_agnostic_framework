...
| **Killarney** (Alliance) | Slurm cluster (same as Vulcan) | **Overflow GPU.** Same cluster type; fewer datasets staged there (stage more if needed). Used when Vulcan priority runs low and we need more GPU. |
...
**FOLD POLICY — we cap at 3 folds.** Always train **folds 0 1 2 only** (`TRAIN_FOLDS="0 1 2"`). If a dataset already has a 4-fold split, **reuse it and just train the first 3 folds** — do **not** create a new split and do **not** train fold 3. (`00_01_train/train_common.sh` honours `TRAIN_FOLDS`; its default is still `"0 1 2 3"` for back-compat, so the per-method wrapper must set `TRAIN_FOLDS="0 1 2"`.)
