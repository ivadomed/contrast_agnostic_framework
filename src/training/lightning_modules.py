...
from src.augmentation.bigaug_augmentations import build_bigaug_augmentation
from src.training.losses import (
    DiceEdgeLoss3D,
    DifferentiableWassersteinLoss,
    GuidanceLoss3D,
    LocalVarianceLoss3D,
    MultiScaleGradientLoss3D,
    RangeLoss,
    TotalVariationLoss3D,
)
from src.augmentation.kornia_augmentations import RandomFourierAmplitude3D, build_kornia_augmentation
...
from src.synthesis.guidance_perturbers import BaseGuidancePerturber, IdentityGuidancePerturber
from src.synthesis.target_generators import BaseTargetGenerator
from src.augmentation.resolution_augmentation import RandomResolutionResampler3D as ResolutionResampler3D
...
class CompiledSynthesisWrapper(nn.Module):
    def __init__(
        ...
        post_resampler: ResolutionResampler3D | None = None,
    ):
        super().__init__()
        ...
        self.post_resampler = post_resampler
    ...
    def forward(self, x: torch.Tensor,
        ...
        if self.post_resampler is not None:
            synthesized = self.post_resampler(synthesized)
...
class CompiledLossWrapper(nn.Module):
    def __init__(
        ...
        guidance_perturber: BaseGuidancePerturber | None,
        apply_guidance_blur: bool,
        edge_map_threshold_low: float = 0.0,
        edge_map_threshold_high: float = 0.0,
        ...
        self.local_var_loss_fn = local_var_loss_fn
        self.target_generator = target_generator
        self.guidance_perturber = guidance_perturber or IdentityGuidancePerturber()
        self.apply_guidance_blur = bool(apply_guidance_blur)
        # > 0 means: add edge map as 3rd input channel with threshold ~ U(low, high)
        self.edge_map_threshold_low = float(edge_map_threshold_low)
        self.edge_map_threshold_high = float(edge_map_threshold_high)
    ...
    def forward(self, x: torch.Tensor,
                ...
                w_guide_blur: float, w_guide_sharp: float, w_local_var: float = 0.0,
        ...
        if self.edge_map_threshold_high > 0.0:
            B = x.shape[0]
            t = torch.rand(B, device=x.device) * (self.edge_map_threshold_high - self.edge_map_threshold_low) + self.edge_map_threshold_low
            sobel_threshold = t.view(B, 1, 1, 1, 1).to(dtype=x.dtype)
            edge_map_input = self.edge_loss_fn._edge_map_with_threshold(x.float(), sobel_threshold).to(x.dtype)
            edge_map_input = edge_map_input.contiguous(memory_format=torch.channels_last_3d)
            if self.no_image_input:
                model_input = torch.cat([blurred_guidance, edge_map_input], dim=1)
            else:
                model_input = torch.cat([x, blurred_guidance, edge_map_input], dim=1)
        else:
        ...
        edge_loss = self.edge_loss_fn(synthesized_01, x, threshold=sobel_threshold)
        ...
        local_var_loss = self.local_var_loss_fn(synthesized_01, x)
        ...
        return {
            ...
            "local_var_loss": local_var_loss,
            "blurred_guidance": blurred_guidance,
            "edge_map_input": edge_map_input,
...
class MRISynthesisLightning(pl.LightningModule):
    ...
    def __init__(self, cfg: DictConfig):
        ...
        _edge_loss_type = str(getattr(cfg.model.generator, "edge_loss", "dice"))
        if _edge_loss_type == "ms_grad":
            self.edge_loss_fn = MultiScaleGradientLoss3D()
        else:
            self.edge_loss_fn = DiceEdgeLoss3D(
                edge_threshold=float(getattr(cfg.model.generator, "edge_threshold", 0.05)),
                sigmoid_scale=float(getattr(cfg.model.generator, "sigmoid_scale", 12.0)),
            )
        self.tv_loss_fn = TotalVariationLoss3D()
        self.range_loss_fn = RangeLoss(min_value=-1.0, max_value=1.0)
        self.local_var_loss_fn = LocalVarianceLoss3D(
            kernel_size=int(getattr(cfg.model.generator, "local_var_kernel_size", 5))
        )
        ...
        _edge_thresh_range = getattr(cfg.model.generator, "edge_map_threshold_range", None)
        _edge_thresh_low = float(_edge_thresh_range[0]) if _edge_thresh_range else 0.0
        _edge_thresh_high = float(_edge_thresh_range[1]) if _edge_thresh_range else 0.0
        ...
        self.compiled_wrapper = CompiledLossWrapper(
            ...
            self.local_var_loss_fn,
            target_generator,
            guidance_perturber,
            self._generator_apply_guidance_blur(),
            edge_map_threshold_low=_edge_thresh_low,
            edge_map_threshold_high=_edge_thresh_high,
            no_image_input=bool(getattr(cfg.model.generator, "no_image_input", False)),
        )
    ...
    def _default_target_generator_cfg(self, version: str) -> dict[str, Any]:
        ...
        if version in ("v8", "v9", "v10", "v11"):
            return {"_target_": "src.synthesis.target_generators.V8GridTargetGenerator", "grid_size": [4, 4, 4]}
        if version == "v15":
            return {
                "_target_": "src.synthesis.target_generators.V15GridTargetGenerator",
                "grid_size": [4, 4, 4],
                "background_threshold": 0.01,
            }
        if version == "v17_micro_anchor":
            return {
                "_target_": "src.synthesis.target_generators.V17MicroAnchorTargetGenerator",
                "tau": 0.05,
                "num_peaks": 4,
                "background_threshold": 0.01,
            }
        return {"_target_": "src.synthesis.target_generators.LegacyChunkTargetGenerator"}
    ...
    def training_step(self, batch: dict[str, torch.Tensor], batch_idx: int) -> torch.Tensor:
        ...
        w_local_var=float(getattr(self.cfg.model.generator.loss_weights, "local_var", 0.0))

        outs = self.compiled_wrapper(
            x, num_bins, num_chunks, dark_threshold,
            guidance_blur_k, guidance_blur_s,
            w_edge, w_tv, w_range, w_wass,
            w_guide_blur, w_guide_sharp, w_local_var, labels=batch.get("label", None)
        )
        ...
        if batch_idx % int(self.cfg.training.generator.log_aux_every_n_steps) == 0:
            ...
            self.log("train/local_var_loss", outs["local_var_loss"], on_step=True, on_epoch=False)
        ...
        if (
            ...
            self._log_train_images(
                ...
                batch_idx=batch_idx,
                local_var_loss_fn=self.local_var_loss_fn,
                edge_map_input=outs["edge_map_input"],
                sobel_threshold=outs["sobel_threshold"],
            )
    ...
    def _log_train_images(
        ...
        *,
        ...
        batch_idx: int,
        local_var_loss_fn: LocalVarianceLoss3D | None = None,
        edge_map_input: torch.Tensor | None = None,
        sobel_threshold: torch.Tensor | None = None,
    ) -> None:
        with torch.no_grad():
            ...
            if sobel_threshold is not None:
                t = sobel_threshold[:max_items].detach().float()
                x_edge_vol = self.edge_loss_fn._edge_map_with_threshold(x[:max_items].detach().float(), t)
                s_edge_vol = self.edge_loss_fn._edge_map_with_threshold(synthesized_01[:max_items].detach().float(), t)
            else:
                x_edge_vol = self.edge_loss_fn._edge_map(x[:max_items].detach().float())
                s_edge_vol = self.edge_loss_fn._edge_map(synthesized_01[:max_items].detach().float())
            xe_slices = x_edge_vol[:, :, depth_idx].cpu().clamp(0.0, 1.0)
            se_slices = s_edge_vol[:, :, depth_idx].cpu().clamp(0.0, 1.0)

            if edge_map_input is not None:
                em_slices = edge_map_input[:max_items, :, depth_idx].detach().float().cpu().clamp(0.0, 1.0)
                rows = []
                for idx in range(max_items):
                    rows.extend([x_slices[idx], g_slices[idx], em_slices[idx], s_slices[idx], xe_slices[idx], se_slices[idx]])
                nrow = 6
                caption = "input | guidance | edge_input | synth | input_edges(fixed) | synth_edges(fixed)"
            else:
                rows = []
                for idx in range(max_items):
                    rows.extend([x_slices[idx], g_slices[idx], s_slices[idx], xe_slices[idx], se_slices[idx]])
                nrow = 5
                caption = "input | guidance | synth | input_edges | synth_edges"

            grid_tensor = torch.stack(rows, dim=0)
            grid = make_grid(grid_tensor, nrow=nrow, padding=2)
            ...
            log_dict = {
                "train/slice_grid": wandb.Image(grid_np, caption=caption),
                "train/hist_overlay": wandb.Image(Image.fromarray(hist_np)),
                "global_step": self.global_step,
                "batch_idx": batch_idx,
            }

            if local_var_loss_fn is not None:
                xf = x[:max_items].detach().float()
                sf = synthesized_01[:max_items].detach().float()
                x_std_vol = local_var_loss_fn.local_std(xf)   # (B,1,D,H,W)
                s_std_vol = local_var_loss_fn.local_std(sf)
                diff_vol = (s_std_vol - x_std_vol).abs()

                def _norm_slice(vol):
                    sl = vol[:, :, depth_idx].cpu()
                    return (sl / sl.amax(dim=(1, 2, 3), keepdim=True).clamp_min(1e-8)).clamp(0.0, 1.0)

                xv_sl = _norm_slice(x_std_vol)
                sv_sl = _norm_slice(s_std_vol)
                dv_sl = _norm_slice(diff_vol)

                var_rows = []
                for idx in range(max_items):
                    var_rows.extend([xv_sl[idx], sv_sl[idx], dv_sl[idx]])
                var_grid = make_grid(torch.stack(var_rows, dim=0), nrow=3, padding=2)
                var_np = var_grid.permute(1, 2, 0).numpy()
                if var_np.shape[-1] == 1:
                    var_np = var_np[..., 0]
                log_dict["train/local_var_grid"] = wandb.Image(
                    var_np,
                    caption="input_std | output_std | |diff|  (normalized per-image)",
                )
    ...
    def _ensure_gpu_aug(self) -> None:
        ...
        if task == "segmenter":
            ...
            if version == "v20_1" or version == "v20":
                from src.synthesis.intensity_ops import PartialSynthSegAugmentation3D
                ...
                return
            if version == "v21":
                from src.synthesis.intensity_ops import SparseSynthSegAugmentation3D
...
# Removed standard compile from setup to use CompiledSegmenterWrapper
        ...
        _res_cfg = getattr(self.cfg.model.segmenter, "resolution_resampler", None)
        if _res_cfg is not None:
            _post_resampler = ResolutionResampler3D(
                zoom_range=(float(_res_cfg.zoom_min), float(_res_cfg.zoom_max)),
                anisotropy_prob=float(getattr(_res_cfg, "anisotropy_prob", 0.5)),
                apply_prob=float(getattr(_res_cfg, "apply_prob", 1.0)),
            )
        else:
            _post_resampler = None
        ...
        self.compiled_synthesis = CompiledSynthesisWrapper(
            ...
            post_resampler=_post_resampler,
        )
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        if self._is_multiclass:
            ...
            if not class_names:
                if str(getattr(self.cfg.data, "name", "")) == "brats2017":
